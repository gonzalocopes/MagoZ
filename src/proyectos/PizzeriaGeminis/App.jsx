...
function App() {
  ...
  const addToCart = (product) => {
    ...
    if (isEmpanadaPack) {
      ...
      // 👇 Configuramos los items del modal (empanadas)
      setUpsellItems(empanadaFlavors);
      setShowUpsell(true);
      return;
    }

    // ✅ TARTAS (Individual / Entera) -> Elegir 1 sabor
    if (product.category === "Tartas" && product.upsell) {
      const lineId = uid();
      ...
      setCart((prev) => [
        ...prev,
        {
          ...
          lineId,
          qty: 1,
          pack: { size: 1, items: {} }, // Pack de 1 unidad (sabor)
        },
      ]);
      ...
      setActiveLineId(lineId);
      setUpsellMode("pack");
      setRequiredPackCount(1);
      // 👇 Configuramos los items del modal (sabores de tarta)
      setUpsellItems(tartaFlavors);
      setShowUpsell(true);
      return;
    }
  ...
  const handleEdit = (item) => {
    ...
    if (item.id === "promo-1-emp") {
      ...
      setUpsellItems(empanadaFlavors);
      setShowUpsell(true);
    } else if (item.category === "Tartas") {
      // Tarta (Individual o Entera)
      setUpsellMode("pack");
      setRequiredPackCount(1);
      setUpsellItems(tartaFlavors);
      setShowUpsell(true);
  ...
  return (
    <div className="bg-body-tertiary min-vh-100">
      ...
      <UpsellModal
        ...
        // empanadaFlavors ya no se usa, now upsellItems is generic
