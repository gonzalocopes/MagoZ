...
export default function UpsellModal({
  ...
  useEffect(() => {
    // Si es Pasta, NO generamos lista de "ingredientes" para sacar.
    if (lastProduct?.category === "Pastas") {
      setIngredients([]);
      setAddedIds([]);
      return;
    }
    ...
    if (show && lastProduct?.description) {
      ...
      // 2. Separamos por comas O por signos más (+)
      ...
      finalIngredients = finalIngredients
        ...
        .filter((i) => {
          ...
          // Excluir Pan, Carnes (core) y Cheddar (core)
          if (lower.includes("pan de papa")) return false;
          
          // Filtramos variaciones de carne
          if (lower.includes("carne")) return false;
          if (lower.match(/^\d+\s*carne/)) return false; // ej: "1 carne"
          if (lower.includes("medallón")) return false;

          // Filtramos cheddar
          if (lower.includes("cheddar")) return false;
  ...
  let icon = "🍽️";
  if (isHamburguesa) icon = "🍔";
  if (isPasta) icon = "🍝";

  let title = `¿Le sumamos algo a tu pedido? ${icon}`;
  if (isHamburguesa) title = `Personalizá tu Hamburguesa ${icon}`;
  if (isPasta) title = `Elegí el sabor ${icon}`;
        ...
        <div className="modal-content">
          ...
          <div className="modal-body upsell-scroll-area">
            ...
            <h6 className="fw-bold mb-2">
              {isPasta ? "Seleccioná tu gusto:" : "¿Querés agregar algo más?"}
            </h6>
